...
.get(cors.cors,(req,res,next) => {
   /* Dishes.find({}) */
   Dishes.find(req.query) ////pour angular
...
.post(cors.corsWithOptions,authenticate.verifyUser,(req, res, next) => {
    ...
    if(dish != null){
        ////req.user._id ====>authenticate.verifyUser(passport loded the user)
    ...
    .then((dish)=>{
        if(dish != null && dish.comments.id(req.params.commentId) !=null){
            ...
            if(req.body.comment){
                ...
                dish.save()
                .then((dish) => {
                    Dishes.findById(dish.id).populate('comments.author')
                    .then((dish)=>{
                        res.statusCode = 200;
                        res.setHeader('Content-Type', 'application/json');
                        res.json(dish); 
